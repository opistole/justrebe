  ...
  function showPostSubmit(opts, readiness, signupId) {
    ...
    if (readiness === 'ready_to_pay') {
      zone.innerHTML =
        ...
        '<p class="post-recap">ReBe ReFresh · 5 weeks · ' + price + '</p>' +
      ...
      zone.innerHTML =
        '<h3 class="post-h">We’ll be in touch.</h3>' +
        '<p>Thanks for your interest in ReBe ReFresh. We’ll reach out within 48 hours with more about the cohort, what to expect, and how to take the next step when you’re ready.</p>' +
        '<p class="post-help">Have a specific question? Just email <a href="mailto:' + contact + '">' + contact + '</a> and we’ll answer directly.</p>';
    } else {
      zone.innerHTML =
    ...
    form.addEventListener('submit', function (e) {
      ...
      supabase
        ...
        .then(function (res) {
          ...
          var signupId = res.data;
          showPostSubmit(opts, payload.readiness, signupId);
          ...
          fetch('/api/notify', {
            ...
            body: JSON.stringify({
              kind: 'cohort_signup',
              record: Object.assign({}, payload, { id: signupId })
            })
          }).catch(function (e) { console.error('notify (cohort_signup) failed:', e); });
        })
        .catch(function (err) {
