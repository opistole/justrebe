...
(function () {
  ...
  function showPostSubmit(opts, payload, requestId) {
    ...
    zone.innerHTML =
      ...
      (payload.preferred_confidant === 'Any (Choose for me)' || payload.preferred_confidant === 'No preference'
  ...
  window.initConfidantForm = function (opts) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      // Call the SECURITY DEFINER RPC instead of a direct INSERT — the RPC
      // bypasses the RLS read-back issue (anon doesn't have SELECT on this
      // table, so .insert().select('id') was failing on the response read).
      supabase
        .rpc('submit_confidant_request', {
          p_name:                payload.name || null,
          p_email:               payload.email || null,
          p_phone:               payload.phone || null,
          p_preferred_confidant: payload.preferred_confidant || null,
          p_situation:           payload.situation || null,
          p_best_times:          payload.best_times || null
        })
        .then(function (res) {
          if (res.error || !res.data || res.data.success === false) {
            console.error('RPC submit_confidant_request error', res.error || res.data);
          ...
          showPostSubmit(opts, payload, requestId);
          // Fire the confirmation email + admin notification (don't block UI on this)
          fetch('/api/notify', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              kind: 'confidant_request',
              record: Object.assign({}, payload, { id: requestId })
            })
          }).catch(function (e) { console.error('notify (confidant_request) failed:', e); });
        })
        .catch(function (err) {
